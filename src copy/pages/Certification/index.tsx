...
const Certification = () => {
    ...
    function constructOthersImages() {
        if (window.innerWidth > 640) {
            const result = images.nameFile.map((item, index) =>
                <div key={index}>
                    <img data-index={index} id={`imgCertification`} src={`${basicrUrl}/${item}`} alt="certificado" className="w-full max-w-[10rem] cursor-pointer border-secondary" onClick={event => setPositionArrayFiles(index)} />
                </div>
            )
            return result
        } else {
    ...
    return (
        <main className="flex flex-col gap-3 items-center w-full">
            <section className="flex justify-center w-full bg-primary py-6 px-6 sm:px-20">
            ...
            <section className="flex flex-col gap-4 max-w-4xl w-full px-8 sm:px-20">
                ...
                <Link
                    to={`/`}
                    className="w-fit bg-primary px-4 py-1 text-white rounded-lg font-medium text-xl hover:bg-cor-hover">
                    Voltar
                </Link>
            </section>
        </main>
