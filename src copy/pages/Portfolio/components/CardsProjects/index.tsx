...
function CardsProjects({ folder, imageName, qtdImagesInFolder,
	...
	return (
		<div className="flex flex-col gap-4 border border-primary h-full rounded-lg bg-white dark:bg-primary px-8 py-4 shadow-gray-600 dark:shadow-none shadow-lg lg:opacity-60 hover:opacity-100 hover:animate-growProjects">
			<div className="flex flex-col gap-2">
				<h2 className="font-semibold text-xl dark:text-gray-200">{title}</h2>
				<img src={`../images/${folder}/${imageName}.jpg`} alt={altProject} height={225} onClick={event => openImageOnScreen()} className="rounded-lg border-primary dark:border-cor-darkHover border-2 dark:border-4 cursor-pointer hover:animate-shadowPopProject" />
			</div>
				...
				<div className='flex justify-between items-end'>
					<div className="flex flex-col gap-4">
						<div className='flex items-start gap-5'>
							{downloadApp ?
								<button onClick={event => handleDownloadApp(downloadApp)} className='flex items-center cursor-pointer gap-2 bg-primary py-1 px-3 rounded-md hover:bg-cor-hover dark:bg-cor-darkSecundaria dark:hover:bg-cor-darkHover'>
									<IoMdDownload size={18} className="text-white dark:text-gray-200" />
									<p className="text-white text-sm font-medium">Baixar APK</p>
								</button> :
								<>
									<a href={linkViewProject} rel="noreferrer" target="_blank" className='flex items-center gap-2 bg-primary py-1 px-3 rounded-md hover:bg-cor-hover dark:bg-cor-darkSecundaria dark:hover:bg-cor-darkHover'>
										<FaRegEye size={18} className="text-white dark:text-gray-200" />
										<p className="text-white text-sm font-medium">Visualizar</p>
									</a>
									<a href={linkViewCode} rel="noreferrer" target="_blank" className='flex items-center gap-2 bg-primary py-1 px-3 rounded-md hover:bg-cor-hover dark:bg-cor-darkSecundaria dark:hover:bg-cor-darkHover'>
